...
NetDeviceContainer
LteHelper::InstallEnbDevice (NodeContainer c,Ptr<MyGymEnv> son_server)
{
  NS_LOG_FUNCTION (this);
...
NetDeviceContainer
LteHelper::InstallUeDevice (NodeContainer c, Ptr<MyGymEnv> son_server)
{
  NS_LOG_FUNCTION (this);
  NetDeviceContainer devices;
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
    {
      Ptr<Node> node = *i;
      Ptr<NetDevice> device = InstallSingleUeDevice (node, son_server);
      devices.Add (device);
    }
  return devices;
}
...
Ptr<NetDevice>
LteHelper::InstallSingleEnbDevice (Ptr<Node> n, Ptr<MyGymEnv> son_server, int totalN)
{
  NS_LOG_FUNCTION (this << n);
...
Ptr<NetDevice>
LteHelper::InstallSingleUeDevice (Ptr<Node> n, Ptr<MyGymEnv> son_server)
{
  NS_LOG_FUNCTION (this);
