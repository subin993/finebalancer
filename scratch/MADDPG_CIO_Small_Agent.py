...
if __name__ == "__main__" :
    ...
    for e in range(EPISODES):
        ...
        state1 = np.concatenate( (state1_1, state2_1, state3_1, state4_1), axis=None )
        state2 = np.concatenate( (state1_2, state2_2, state3_2, state4_2), axis=None )
        state3 = np.concatenate( (state1_3, state2_3, state3_3, state4_3), axis=None )
        print("state3: ",state3)
        print("state4: ",state4)
        ...
        for time in range(max_env_steps): 
            ...
            for agent_id, agent in enumerate(agents):
                if (np.random.rand() <= epsilon) :
                    print("Random Action")
                    action = round(random.uniform(-6,6),0)
            ...
            env_actions.insert(0,0)
            env_actions.insert(0,0)
            for i in range(5):
                env_actions.append(0.0)
            ...
            # Save results
            # if(time == 0):
            #     with open("/home/mnc/ns3-son/Baseline/NS3_SON/Reward_1154.txt", 'w',encoding="UTF-8") as k:
            #         k.write(str(RewardSum)+"\n")
            # else:
            #     with open("/home/mnc/ns3-son/Baseline/NS3_SON/Reward_1154.txt", 'a',encoding="UTF-8") as k:
            #         k.write(str(RewardSum)+"\n")

            next_state1 = np.concatenate( (next_state1_1, next_state2_1, next_state3_1, next_state4_1), axis=None )
            next_state2 = np.concatenate( (next_state1_2, next_state2_2, next_state3_2, next_state4_2), axis=None )
            next_state3 = np.concatenate( (next_state1_3, next_state2_3, next_state3_3, next_state4_3), axis=None )
            ...
            if(time != 0) :
                replay_buffers[0].add(state1, actions[0], next_state1, R_rewards[0])
                replay_buffers[1].add(state2, actions[1], next_state2, R_rewards[1])
                replay_buffers[2].add(state3, actions[2], next_state3, R_rewards[2])
            ...
            if replay_buffers[0].size >= batch_size:
                ...
                for agent in agents:
                    print("Agent Id: ",agent.agent_id, " Loss: ", agent.actor_loss)
