        ...
        for time in range(max_env_steps): 
            ...
            for agent_id, agent in enumerate(agents):
                if (np.random.rand() <= epsilon) :
                    ...
                    action = random.uniform(-6,6)
                    env_action = round(action, 4)
                    
                    actions.insert(agent_id,np.array(action))

                    env_actions.insert(agent_id,0.0)
                    env_actions.insert(agent_id+3,env_action)
                    ...
                    env_action = (np.round(action,4)).astype(float)
                    env_action = float(action)
                    env_actions.insert(agent_id,0.0)
                    env_actions.insert(agent_id+3,env_action)
            ...
            # Save results
            # if(time == 0):
            #     with open("/home/mnc/ns3-son/Baseline/NS3_SON/Reward_1168.txt", 'w',encoding="UTF-8") as k:
            #         k.write(str(RewardSum)+"\n")
            # else:
            #     with open("/home/mnc/ns3-son/Baseline/NS3_SON/Reward_1168.txt", 'a',encoding="UTF-8") as k:
            #         k.write(str(RewardSum)+"\n")
